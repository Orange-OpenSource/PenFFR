...
quantffr <- function(Y.mat, t.mat = "all.ok", X_fd.list, X.scal = data.frame(), n.order = 4, nbasis = "all.ok", pen = T, tau = 0.5){
  '
  ...
  if (pen == F){
    ...
    my_formula <- as.formula(paste("output ~ 0", paste(colnames(data)[-(1:3)],
                                                       collapse = " + "),
                                   sep = " + "))
    model <- rq(my_formula, data = cbind.data.frame(data), tau = tau)
    ...
    fofreg <- Quantsim(data = data, lams = seq(0.5, 5.0,length.out = n.lam),
...
pred.quantffr <- function(model, newX_fd.list = list(), newX.scal = data.frame(), t.mat = "all.ok"){
  ...
  if (length(newX_fd.list) == 0) {
    ...
    # prediction ----
    pred <-  matrix(predict(model$model, newdata = data, type = "response"), nrow = n,
                    byrow = T)
  }
